...
// Local + OAuth (Google, Spotify, Discord)
// Upgrade to Supabase/Firebase when hosted
// ============================================
const Auth = (() => {
  ...
  // Simple hash (not for production — use bcrypt server-side)
  ...
  async function signup(name, email, password){
    ...
    const user={ id:'local_'+Date.now(), name, email:email.toLowerCase(), hash,
      avatar:null, provider:'email', createdAt:new Date().toISOString() };
    users.push(user);
    saveUsers(users);
    setSession(user);
  ...
  async function login(email, password){
    ...
    const hash=await hashPw(password);
    if(hash!==user.hash){ Notify.error('Incorrect password'); return false; }
    setSession(user);
    Notify.success(`Welcome back, ${user.name}! 🎵`);
    return true;
  }

  // ── OAuth (opens popup — needs real client IDs when hosted) ──
  function oauthLogin(provider){
    const configs = {
      google: {
        // Replace with real Google OAuth client ID
        url: `https://accounts.google.com/oauth2/v2/auth?response_type=token&client_id=YOUR_GOOGLE_CLIENT_ID&redirect_uri=${encodeURIComponent(window.location.origin+'/oauth/google')}&scope=email%20profile`,
        name: 'Google'
      },
      discord: {
        // Replace with real Discord app client ID
        url: `https://discord.com/api/oauth2/authorize?client_id=YOUR_DISCORD_CLIENT_ID&response_type=token&redirect_uri=${encodeURIComponent(window.location.origin+'/oauth/discord')}&scope=identify%20email`,
        name: 'Discord'
      },
      spotify: {
        // Reuse Spotify from spotify.js
        name: 'Spotify'
      }
    };
  ...
  function setSession(user){
    currentUser={ id:user.id, name:user.name, email:user.email,
      avatar:user.avatar||null, provider:user.provider };
  ...
  function showModal(tab='login'){
      ...
      modal.querySelectorAll('.auth-input').forEach(inp=>{
        inp.addEventListener('keydown',e=>{
          if(e.key==='Enter'){
            ...
            activeForm?.querySelector('.auth-submit')?.click();
          }
        });
      });
    ...
    modal.querySelectorAll('.auth-form').forEach(f=>f.classList.add('hidden'));
    const formId=tab==='login'?'authFormLogin':'authFormSignup';
  ...
  function showProfileMenu(){
    let menu=document.getElementById('authProfileMenu');
    if(menu){menu.remove();return;}
    menu=document.createElement('div');
    menu.id='authProfileMenu';
    menu.className='auth-profile-menu';
    menu.innerHTML=`
      <div class="apm-header">
        <div class="apm-avatar">${currentUser.name[0].toUpperCase()}</div>
        <div>
          <div class="apm-name">${esc(currentUser.name)}</div>
          <div class="apm-email">${esc(currentUser.email||currentUser.provider)}</div>
        </div>
      </div>
      <div class="apm-divider"></div>
      <button class="apm-item" onclick="Auth._editProfile()">✏️ Edit Profile</button>
      <button class="apm-item" onclick="Auth._logout()">↪ Sign Out</button>
    `;
    document.body.appendChild(menu);
    // Position near auth button
    const btn=document.getElementById('authBtn');
    const r=btn.getBoundingClientRect();
    menu.style.cssText=`position:fixed;top:${r.bottom+6}px;right:${window.innerWidth-r.right}px;z-index:8000`;
    setTimeout(()=>document.addEventListener('click',()=>menu.remove(),{once:true}),10);
  }

  function editProfile(){
    const newName=prompt('Display name:',currentUser.name);
    if(!newName?.trim()) return;
    currentUser.name=newName.trim();
    saveSession(currentUser);
    const users=getUsers();
    const u=users.find(u=>u.id===currentUser.id);
    if(u){u.name=newName.trim();saveUsers(users);}
    updateUI();
